export const PATH = {
  API: "api",
  OAUTH: "oauth",
  FACEBOOK: "facebook",
  GOOGLE: "google",
  APPLE: "apple",
  CREDS: "creds",
  LOGIN: "login",
  REGISTRATION: "registration",
  API_DOCS: "api-docs",
  FIREBASE: "firebase",
  SUBSCRIBE: "subscribe",
  PUSH: "push",
  USER: "user",
  PROFILE: "profile",
  AUTH: "auth",
  CONTACTS: "contacts",
  NOTIFICATIONS: "notifications",
  CHECK: "check",
  COMMENTS: "comments",
  HINTS: "hints",
  RECORD: "record",
  PUBLISH: "publish",
  REC_COMMENTS: "rec-comments",
  LOGS: "logs",
  SERVER_SIDE: "serverSide",
  SEND_SMS: "sendSms",
  VERIFY_CODE: "verifyCode",
  PHONE: "phone",
  IMPORT: "contactsImport",
  FORGOT_PASSWORD: "forgot-password",
  RESET_PASSWORD: "reset-password",
  BLOCK: "block",
  FEATURED: "featured",
  AVAILABLE: "available",
  REPORT: "report",
  INVITE: "invite",
  FILTER: "filter",
  ALL: "all",
  DELETE: "delete",
  UPDATE: "update",
  SHARED: "shared",
  STATUS: "status",
  NEW: "new",
};

export const API = {
  OAUTH_FACEBOOK: `${PATH.API}/${PATH.OAUTH}/${PATH.FACEBOOK}`,
  OAUTH_GOOGLE: `${PATH.API}/${PATH.OAUTH}/${PATH.GOOGLE}`,
  OAUTH_APPLE: `/${PATH.API}/${PATH.OAUTH}/${PATH.APPLE}`,
  OAUTH_CREDS: `${PATH.API}/${PATH.OAUTH}/${PATH.CREDS}`,
  LOGIN: `${PATH.API}/${PATH.LOGIN}`,
  REGISTRATION: `${PATH.API}/${PATH.REGISTRATION}`,
  SUBSCRIBE_TO_FIREBASE_PUSHES: `${PATH.API}/${PATH.FIREBASE}/${PATH.SUBSCRIBE}`,
  SEND_FIREBASE_PUSH: `${PATH.API}/${PATH.FIREBASE}/${PATH.PUSH}`,
  USER: `${PATH.API}/${PATH.USER}`,
  USER_UPDATE: `${PATH.API}/${PATH.USER}/${PATH.UPDATE}`,
  USER_HINTS: `${PATH.API}/${PATH.USER}/${PATH.HINTS}`,
  USER_PROFILE: `${PATH.API}/${PATH.PROFILE}`,
  AVCORE_CREDS: `${PATH.AUTH}`,
  USER_CONTACTS: `${PATH.API}/${PATH.CONTACTS}`,
  INVITE_CONTACTS: `/${PATH.API}/${PATH.CONTACTS}/${PATH.INVITE}`,
  NOTIFICATIONS: `${PATH.API}/${PATH.NOTIFICATIONS}`,
  CHECK_NOTIFICATIONS: `${PATH.API}/${PATH.NOTIFICATIONS}/${PATH.CHECK}`,
  COMMENTS: `${PATH.API}/${PATH.COMMENTS}`,
  RECORDING_COMMENTS: `${PATH.API}/${PATH.COMMENTS}/${PATH.REC_COMMENTS}`,
  RECORD: `${PATH.API}/${PATH.RECORD}`,
  RECORD_PUBLISH: `${PATH.API}/${PATH.RECORD}/${PATH.PUBLISH}`,
  ALL_RECORD_IDS: `/${PATH.API}/${PATH.RECORD}/${PATH.ALL}`,
  SEND_LOGS: `${PATH.API}/${PATH.LOGS}`,
  LOG_ON_SERVER: `${PATH.API}/${PATH.LOGS}/${PATH.SERVER_SIDE}`,
  SEND_SMS: `${PATH.API}/${PATH.USER}/${PATH.SEND_SMS}`,
  VERIFY_CODE: `${PATH.API}/${PATH.USER}/${PATH.VERIFY_CODE}`,
  UPDATE_PHONE: `${PATH.API}/${PATH.USER}/${PATH.PHONE}`,
  IMPORT_CONTACTS: `${PATH.API}/${PATH.CONTACTS}/${PATH.IMPORT}`,
  BLOCK: `/${PATH.API}/${PATH.BLOCK}`,
  FEATURED: `/${PATH.API}/${PATH.FEATURED}`,
  RECORD_NEW: `/${PATH.API}/${PATH.RECORD}/${PATH.NEW}`,
  RECORD_AVAILABLE: `/${PATH.API}/${PATH.RECORD}/${PATH.AVAILABLE}`,
  RECORD_BLOCK: `/${PATH.API}/${PATH.RECORD}/${PATH.BLOCK}`,
  RECORD_FILTER: `/${PATH.API}/${PATH.RECORD}/${PATH.FILTER}`,
  RECORD_FILTER_ALL: `/${PATH.API}/${PATH.RECORD}/${PATH.FILTER}/${PATH.ALL}`,
  GET_VERIFY_CODE: `/${PATH.API}/${PATH.FORGOT_PASSWORD}`,
  RESET_PASSWORD: `/${PATH.API}/${PATH.RESET_PASSWORD}`,
  REPORT: `/${PATH.API}/${PATH.RECORD}/${PATH.REPORT}`,
  DELETE: `/${PATH.API}/${PATH.RECORD}/${PATH.DELETE}`,
  PROFILE_UPDATE: `/${PATH.API}/${PATH.PROFILE}/${PATH.UPDATE}`,
  SHARED: `/${PATH.SHARED}/${PATH.RECORD}`,
  CHECK_FEATURED: `/${PATH.API}/${PATH.FEATURED}/${PATH.CHECK}`,
  UPDATE_FEATURED: `/${PATH.API}/${PATH.FEATURED}/${PATH.UPDATE}`,
  GET_FEATURED: `/${PATH.API}/${PATH.FEATURED}/${PATH.STATUS}`,
};
